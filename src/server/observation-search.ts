/**
 * Scope-aware hybrid retrieval over Observations — the shared retrieval kernel
 * (design §7.1): FTS candidates fused with local semantic candidates via
 * Reciprocal Rank Fusion (RRF), then ordered with a stable tie-break. No import
 * side effects: the DB singleton is constructed by the caller, so this is
 * unit-testable with an injected embedder and an in-memory DB.
 */

import type { MemoryDB } from '../db';
// `extractCjkBigrams` lives in the db layer next to `extractFtsSearchUnits` (same
// FTS tokenization concern) rather than being duplicated here.
...
export interface RetrievalPolicy {
  /** Semantic candidates without an FTS match; only effective for a valid `semantic-en-v1` query. */
  semanticDiscovery: boolean;
  /** Strict cosine lower bound (`score > floor`), protocol-specific: never reuse for raw vectors. */
  semanticFloor: number;
  /** Post-fusion quota for `match_source: "semantic"` only — not a whole-page
   * false-recall guardrail, since `fts` and `hybrid` stay unbounded. */
  ...
  /** Optional CJK two-character discovery leg, off: the measured arms improved
   * recall but exceeded false-recall bounds. */
  bigramAux: boolean;
  ...
  bigramWeight: number;
  /** `always` votes for every bigram match; `discovery-only` only when FTS and
   * semantic search did not already find the record. */
  bigramVote: 'always' | 'discovery-only';
  /** Top semantic candidates retained by the streaming scorer. Lexical candidates
   * are always retained and keep their true global semantic rank. */
  semanticTopK: number;
}
 ...
 * English forms stay FTS-only; unsafe raw-vector reranking is not restored. */
...
/**
 * Map the gray-release switch onto one of the two frozen profiles. Takes a boolean,
 * not `Config`, so the kernel depends on no config layout and a benchmark can select
 * a profile without writing one. No third state and no partial merge: these two
 * profiles are the only combinations measured end to end.
 */
...
/**
 * Candidate ids per streaming chunk. Implementation constant, not a strategy field:
 * it bounds peak memory (`4096 × 384 × 4B ≈ 6.3MB` of vector payload per chunk) and
 * keeps the bound parameter count far below SQLite's 65,535 ceiling. Changing it
 * must not change any returned result — `topk-criteria.md` §3.3 and the
 * chunk-boundary tests pin that.
 */
const SEMANTIC_STREAM_CHUNK = 4096;

/**
 * Score candidate vectors in chunks, retaining only a bounded top set. The old
 * "fetch every blob → convert every one → aggregate every above-floor candidate →
 * sort" path is what made scope-wide scoring cost +720MB at 50,000 records while
 * passing the latency budget (pool round §9). Equivalent to it, not an
 ...
 *    candidate carries its true global rank — one outside the top set gets it from
 *    an exact "how many outrank me" counter, not from its array position.
 *
 * `topK = Infinity` retains everything above the floor: the pre-streaming behavior
 * that phase 1 ships.
 */
function streamScoreCandidates(
  ...
  // Must-keep scores plus "how many candidates outrank me": a counter can only count
  // items seen after it exists, and a must-keep candidate may be scored late.
  ...
  const fetch = (chunk: number[]): { observation_id: number; embedding: Buffer }[] =>
    // Version-filtered: a blob from another model or another text normalization
    // protocol degrades to keyword-only reachability instead of a confident-looking
    // but meaningless score. `raw-v1` and `semantic-en-v1` are both 384 dimensions
    // of the same model, so the filter must be the full space key.
  ...
  for (let i = 0; i < ids.length; i += SEMANTIC_STREAM_CHUNK) {
    for (const row of fetch(ids.slice(i, i + SEMANTIC_STREAM_CHUNK))) {
      ...
      aboveFloor++;
      const item = { id, score };
      // Exact global rank for must-keep candidates: count everything that outranks
      // them, heap-retained or not.
  ...
  // Sorting the heap by the same comparator yields positions that are global ranks,
  // because every candidate ranking above a retained one is also retained.
  ...
  // Must-keep candidates the heap dropped: rank from the exact counter, never `topK + 1`.
...
/** Shared empty maps for the early-return observation payload. */
 ...
 *
 * Centralized so a benchmark arm, a test fixture and any future config path all
 * agree on what "valid" means. The caller decides how to fail — `benchmark/run.ts`
 * exits 2 before seeding, because a silently coerced value produces a full report
 * whose provenance names a policy that never ran, indistinguishable from a real one.
 * Every bound below catches a value that fails silently rather than loudly: a floor
 * outside [-1, 1] means "keep everything"/"keep nothing" while reading like a
 * threshold and `NaN` makes every `score > NaN` false; a fractional
 * `semanticOnlyLimit` makes the reported and the enforced cap differ; `rrfK <= 0`
 * divides by zero or flips the sign so worse ranks score higher; a zero weight
 * disables a whole leg while the report still claims two-leg fusion.
 */
export function validateRetrievalPolicy(policy: RetrievalPolicy): string[] {
  ...
  // phase 3B candidate pool: 0 empties the semantic pool while the FTS union keeps
  // results coming, so the arm reads as a floor or cap result; a fractional value is
  // coerced by SQLite, so the enforced pool and the reported pool differ silently.
  if (
  ...
  // --- phase 3A bigram leg (same reasoning as above) ---
  ...
  if (
    ...
    // 0 drops every bigram while the report claims the leg ran; `1` is "no ceiling".
...
/**
 * Default embedder: the bundled local model, loaded lazily. The dynamic import is
 * load-bearing — a static `../embedding` import pulls `@huggingface/transformers`
 * into every process that touches this module, measured at 54MB RSS / 128ms even
 * when no model is instantiated. After phase 1b the MCP server holds neither the
 * runtime nor a model: it passes a Worker-backed embedder, so one model instance
 * serves every repo sharing a dataDir.
 */
...
/**
 * Returns Observations ranked by weighted RRF over FTS + semantic candidate lists,
 * hard-scoped by `opts.scopeKey`; unavailable embeddings degrade to FTS-only. Every
 * quality parameter comes from {@link RetrievalPolicy}:
 * {@link DEFAULT_RETRIEVAL_POLICY} in production,
...
export async function hybridSearchObservations(
  ...
  // Phase 3C: unbounded by default, matching bootstrap's visible range. Imported
  // rather than repeated because the old `?? 90` existed in four separate layers
  // (kernel, two db methods, MCP), and any one left behind keeps filtering.
  ...
  // Vector space, decided once here and used for both the text we embed and the
  // stored rows we read: no code path can take the query text from one protocol
  // and the space key from another.
  ...
  // A separate leg, not extra rows in `ftsRankMap`. Plan §10 item 3 proposed merging
  // them into the FTS rank; merged, a bigram candidate reads as `match_source:
  // 'fts'`, no cap can single it out, and FTS candidates are never dropped. Measured
  // consequence: one expected-empty query returned 15 records against a registered
  // hard bound of 3 (`benchmark/reports/phase3a-bigram-probe.md`).
  ...
  // match counts when that optional leg is enabled.
  ...
  let comparableVectors = 0;
  // Distinct from `semanticRank.size` once `semanticTopK` truncates: the map holds
  // the retained set, this stays the true above-floor population.
  ...
  // vectors under this protocol?". Its own failure degrades to `null`, never to 0.
  ...
  try {
    ...
    // embedding unavailable — FTS-only fusion below; signal it (§12.4 FTS-only rate).
    deps?.onDegrade?.();
  }
  }

  // Both candidate lists are final here; fusion below only reorders and slices them.
  deps?.onCandidates?.({
  ...
  const scored = [...allIds].map((id) => {
    ...
    // Bigram vote (phase 3A-R2): `discovery-only` suppresses it when another leg
    // already found the record. Decided here, before scoring, not deducted
    // afterwards — a post-hoc deduction leaves the rank already shifted by a term
    // the policy says it should not have had.
    ...
    // semantic+bigram stays `semantic`, inside the semantic quota.
  ...
  scored.sort((a, b) => {
    ...
    if (policy.tieBreak === 'semantic-rank') {
      // Only reached inside the same confidence bucket, i.e. the transposed-hybrid
      // case (single-leg candidates of one kind cannot tie — ranks within a leg are
      // unique). No semantic rank sorts last: no floor clearance, no evidence.
  ...
  for (const s of scored) {
    ...
    // Two independent caps, not a shared budget: a shared one would make each
    // kind's measured worst case depend on how many of the other ranked above it.
