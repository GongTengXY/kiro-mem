...
export interface Config {
  ...
  context: { maxSessions: number; maxOutputBytes: number; includePinned: boolean };
  session: { timeoutMinutes: number; autoComplete: boolean };
  filter: { skipTools: string[]; skipSmallReads: boolean; smallReadThreshold: number };
}
...
export function getDataDir(): string {
  return process.env.KIRO_MEMORY_DATA_DIR || join(process.env.HOME || "~", ".kiro-memory");
}
