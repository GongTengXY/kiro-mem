...
`retrieval.semanticDiscovery: true` 是当前代码默认值：只要 `search` 带上合法的 `semantic_query_en`，即使当前 workspace 的 FTS 一条都没命中，语义腿也会照常执行——于是一个和记录用词完全不同的问法仍然能找回它。该 profile 尚未通过最终 P6 误召回门，稳定对外发布继续阻塞。当前两个数字来自此前的完整链路参数扫描：cosine floor `0.197`，以及**每次搜索最多 2 条纯语义结果**。这个上限约束的是**一个来源标签**，不是整个页面——`fts` 与 `hybrid` 结果从来不受它限制，而默认时间窗放宽之后关键词腿的触及范围也跟着变大了。把它当作"未核实纯语义线索的配额"，不要当作整页安全保证。只靠语义找到的结果标记为 `match_source: "semantic"`，据此行动前应当核实。
