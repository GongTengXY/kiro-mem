...
const R2 = join(DATASET_DIR, 'queries-fts-round-r2.json');
// r2 存在即优先：它是判据 r4 §14 的新轮次 query 集，原集逐字节保留但已不再是冻结候选。
const QUERIES_FILE = arg('queries') ?? (SAMPLE ? join(DATASET_DIR, 'queries-fts-round-sample.json') : (existsSync(R2) ? R2 : join(DATASET_DIR, 'queries-fts-round.json')));
...
for (const q of queries) {
  ...
  // r4 §18：保护线与词面负例的 token 必须带 token_source，取值只能是两种。
  // sample gate 的漏项就是没要求「token 必须存在于 Observation 的实际索引投影」，
  // 这个字段是那条机械门在数据层的落点（可达性本身由 precheck 的 Q1–Q4 实测）。
  if (q.cohort === 'lexical-anchor' || q.cohort === 'protection') {
    if (!['indexed-field', 'synthetic-filler'].includes(q.token_source)) {
      fail(`${at}：token_source 必须是 indexed-field 或 synthetic-filler，实际 ${JSON.stringify(q.token_source)}`);
    }
  }
