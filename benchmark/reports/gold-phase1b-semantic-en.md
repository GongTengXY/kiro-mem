...
| 项 | 值 |
| --- | --- |
| 生成时间 | 2026-07-30T12:56:19.742Z |
| commit | `07f8c50` **（工作区有未提交改动）** |
| 命令行 | `bun run benchmark/run.ts --compressor=gold --protocol=semantic-en --report=benchmark/reports/gold-phase1b-semantic-en.md --json=/tmp/armB.json` |
| 数据集 turns.json | `aa8f9beda2a4`（30 turn） |
| 数据集 queries.json | `95ee03d880a9`（42 query） |
| 脚本 run.ts / scoring.ts / dataset.ts | `5019403756fc` / `bb8e48403d71` / `6ca4fdb75eb6` |
...
| --- | --- | --- | --- | --- | --- | --- |
| q01 FTS 特殊字符查询报错是怎么修的 | relevance | t01 | 1 | hybrid+semantic | 0 | 13.9ms |
| q02 search 为什么会降级成只用 FTS | relevance | t02 | 2 | hybrid+semantic+fts | 0 | 11.5ms |
| q03 hook 超时会不会拖慢 Kiro 启动 | relevance | t03,t19 | 2,5 | hybrid+semantic+fts | 0 | 7.5ms |
| q04 日志里会不会有用户 prompt 正文 | relevance | t04,t19 | 3,2 | hybrid+semantic+fts | 0 | 9.2ms |
...
| q12 重新安装之后为什么还在跑旧代码 | relevance | t14 | 1 | hybrid+semantic | 0 | 8.2ms |
| q13 怎么发现运行中的 Worker 不是当前安装的版本 | relevance | t15 | 1 | hybrid+fts | 0 | 19.7ms |
...
| q16 本地 embedding 模型是什么精度 多少维 | relevance | t23 | 1 | hybrid+fts | 0 | 9.9ms |
| q17 跨项目会不会看到别的仓库的记忆 | relevance | t22 | 1 | hybrid | 0 | 9.5ms |
| q18 质量基准集还剩什么没做 | relevance | t26 | 1 | hybrid+semantic | 0 | 10.0ms |
| q19 虚拟滚动 react-window 帧率优化 | empty | — | 返回 0 条 | — | 0 | 8.9ms |
| q20 401 token 刷新重放请求 | leakage | — | 返回 6 条 | hybrid+fts+semantic | 0 | 7.4ms |
| q21 Kubernetes Ingress 灰度发布配置 | empty | — | 返回 0 条 | — | 0 | 8.1ms |
| q22 支付回调 验签 对账差异 | empty | — | 返回 0 条 | — | 0 | 6.3ms |
| q23 iOS APNs 推送证书 续期 | empty | — | 返回 0 条 | — | 0 | 4.9ms |
| q24 Excel 导出 中文乱码 编码 | empty | — | 返回 0 条 | — | 0 | 2.6ms |
| q25 purge 之后重装会不会残留旧表 | relevance | t09 | 1 | hybrid+fts+semantic | 0 | 13.7ms |
| q26 job 的类型名后来有没有改过 | relevance | t10 | 1 | hybrid+semantic+fts | 0 | 6.2ms |
| q27 干净机器上装完做过验收吗 | relevance | t16 | 1 | hybrid+semantic | 0 | 6.9ms |
| q28 有没有真的跑通一整轮记忆生成 | relevance | t17 | 未命中 | — | 0 | 4.0ms |
| q29 两路检索结果是用什么算法合并的 | relevance | t21 | 未命中 | — | 0 | 4.2ms |
| q30 中文界面和中文提示是怎么支持的 | relevance | t24 | 未命中 | — | 0 | 4.1ms |
| q31 还有哪些类型错误没有修完 | relevance | t25 | 1 | hybrid+semantic | 0 | 10.4ms |
| q32 搜索词里带引号或括号会不会崩 | relevance | t01 | 未命中 | — | 0 | 6.1ms |
| q33 向量算不出来的时候搜索还能不能用 | relevance | t02 | 未命中 | — | 0 | 4.8ms |
| q34 报错日志会不会把模型原始输出写进去 | relevance | t04 | 1 | hybrid+semantic | 0 | 8.1ms |
| q35 改一次版本号要动几个文件 | relevance | t05 | 1 | hybrid+semantic | 0 | 7.1ms |
| q36 连接池里有个子进程死了会怎么处理 | relevance | t07 | 未命中 | — | 0 | 5.2ms |
| q37 模型输出解析不了会不会把这一轮丢掉 | relevance | t08 | 未命中 | — | 0 | 4.8ms |
| q38 单元测试用的数据库文件放在哪里 | relevance | t12 | 4 | hybrid+semantic+fts | 0 | 14.5ms |
| q39 安装时少拷了一个文件会有什么现象 | relevance | t13 | 4 | hybrid+semantic | 0 | 11.9ms |
| q40 会话开头塞进去的索引占多少字节 | relevance | t18 | 未命中 | — | 0 | 5.6ms |
| q41 两个不同仓库的记忆是怎么分开的 | relevance | t22 | 1 | hybrid+semantic | 0 | 9.8ms |
| q42 向量是多少维 用了什么量化 | relevance | t23 | 未命中 | — | 0 | 3.0ms |
