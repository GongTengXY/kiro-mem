...
| 项 | 值 |
| --- | --- |
| 生成时间 | 2026-08-03T12:53:12.719Z |
| commit | `b2e972f` **（工作区有未提交改动）** |
| 命令行 | `bun run benchmark/run.ts --compressor=gold --protocol=semantic-en --semantic-discovery=on --semantic-floor=0.197 --semantic-only-limit=2 --rrf-k=60 --fts-weight=1 --semantic-weight=1 --tie-break=recency --report=benchmark/reports/phase2d/confirm-heldout-baseline.md --json=benchmark/reports/phase2d/confirm-heldout-baseline.json` |
...
| q02 search 为什么会降级成只用 FTS | relevance | t02 | 2 | hybrid+semantic+fts | 0 | 9.8ms |
| q03 hook 超时会不会拖慢 Kiro 启动 | relevance | t03,t19 | 2,5 | hybrid+semantic+fts | 0 | 8.3ms |
| q04 日志里会不会有用户 prompt 正文 | relevance | t04,t19 | 3,2 | hybrid+semantic+fts | 0 | 8.2ms |
| q05 版本号是在哪里统一管理的 | relevance | t05 | 1 | hybrid+semantic | 0 | 5.5ms |
| q06 MCP search 的 limit 最大能传多少 | relevance | t06 | 1 | hybrid+fts+semantic | 0 | 9.8ms |
| q07 ACP 连接池回收失败会不会导致任务永久挂住 | relevance | t07 | 1 | hybrid+semantic+fts | 0 | 9.0ms |
| q08 压缩一直失败最后会写成什么样的记录 | relevance | t08 | 1 | semantic | 0 | 6.2ms |
...
| q11 安装完 Worker 起不来说模块找不到 | relevance | t13 | 1 | hybrid+fts | 0 | 11.6ms |
| q12 重新安装之后为什么还在跑旧代码 | relevance | t14 | 1 | hybrid+semantic | 0 | 7.9ms |
| q13 怎么发现运行中的 Worker 不是当前安装的版本 | relevance | t15 | 1 | hybrid+fts | 0 | 17.9ms |
| q14 注入到会话里的上下文有多大 会不会超预算 | relevance | t18 | 2 | hybrid+semantic | 0 | 11.2ms |
| q15 timeline 的邻居是按什么顺序排的 | relevance | t20 | 1 | hybrid+semantic+fts | 0 | 9.0ms |
| q16 本地 embedding 模型是什么精度 多少维 | relevance | t23 | 1 | hybrid+fts | 0 | 9.0ms |
...
| q23 iOS APNs 推送证书 续期 | empty | — | 返回 0 条 | — | 0 | 4.5ms |
| q24 Excel 导出 中文乱码 编码 | empty | — | 返回 1 条 | semantic | 0 | 4.1ms |
| q25 purge 之后重装会不会残留旧表 | relevance | t09 | 1 | hybrid+semantic | 0 | 8.0ms |
| q26 job 的类型名后来有没有改过 | relevance | t10 | 1 | hybrid+semantic+fts | 0 | 7.2ms |
| q27 干净机器上装完做过验收吗 | relevance | t16 | 1 | hybrid+semantic | 0 | 9.3ms |
| q28 有没有真的跑通一整轮记忆生成 | relevance | t17 | 2 | semantic | 0 | 6.5ms |
| q29 两路检索结果是用什么算法合并的 | relevance | t21 | 1 | semantic | 0 | 6.6ms |
| q30 中文界面和中文提示是怎么支持的 | relevance | t24 | 1 | semantic | 0 | 6.5ms |
| q31 还有哪些类型错误没有修完 | relevance | t25 | 1 | hybrid+semantic | 0 | 8.5ms |
| q32 搜索词里带引号或括号会不会崩 | relevance | t01 | 1 | semantic | 0 | 6.5ms |
| q33 向量算不出来的时候搜索还能不能用 | relevance | t02 | 未命中 | semantic | 0 | 5.7ms |
| q34 报错日志会不会把模型原始输出写进去 | relevance | t04 | 1 | hybrid+semantic | 0 | 8.1ms |
| q35 改一次版本号要动几个文件 | relevance | t05 | 1 | hybrid+semantic | 0 | 6.9ms |
| q36 连接池里有个子进程死了会怎么处理 | relevance | t07 | 1 | semantic | 0 | 6.9ms |
| q37 模型输出解析不了会不会把这一轮丢掉 | relevance | t08 | 未命中 | semantic | 0 | 6.6ms |
| q38 单元测试用的数据库文件放在哪里 | relevance | t12 | 4 | hybrid+semantic+fts | 0 | 12.7ms |
| q39 安装时少拷了一个文件会有什么现象 | relevance | t13 | 未命中 | hybrid+semantic | 0 | 12.8ms |
| q40 会话开头塞进去的索引占多少字节 | relevance | t18 | 1 | semantic | 0 | 6.8ms |
| q41 两个不同仓库的记忆是怎么分开的 | relevance | t22 | 1 | hybrid+semantic | 0 | 9.8ms |
| q42 向量是多少维 用了什么量化 | relevance | t23 | 1 | semantic | 0 | 5.8ms |
