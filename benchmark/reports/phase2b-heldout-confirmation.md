...
| 项 | 值 |
| --- | --- |
| 生成时间 | 2026-07-31T13:10:57.025Z |
| commit | `b2e972f` **（工作区有未提交改动）** |
| 命令行 | `bun run benchmark/run.ts --compressor=gold --protocol=semantic-en --semantic-discovery=on --semantic-floor=0.197 --semantic-only-limit=2 --report=benchmark/reports/phase2b-heldout-confirmation.md --json=benchmark/reports/phase2b-heldout-confirmation.json` |
...
| bootstrap 构建 < 100ms 且在预算内 | 11.4ms / 3096B | ✅ |
...
| 跨 workspace 泄漏条数 | 0 |
| FTS-only 降级次数 | 0 / 42 |
| search p50 / p95 | 8.0ms / 12.6ms |
| bootstrap 当前 scope | 3096B / 11.4ms（预算 8192B） |
...
| --- | --- | --- | --- | --- | --- | --- |
| q01 FTS 特殊字符查询报错是怎么修的 | relevance | t01 | 1 | hybrid+semantic | 0 | 13.4ms |
| q02 search 为什么会降级成只用 FTS | relevance | t02 | 2 | hybrid+semantic+fts | 0 | 11.2ms |
...
| q07 ACP 连接池回收失败会不会导致任务永久挂住 | relevance | t07 | 1 | hybrid+semantic+fts | 0 | 9.6ms |
...
| q13 怎么发现运行中的 Worker 不是当前安装的版本 | relevance | t15 | 1 | hybrid+fts | 0 | 20.2ms |
| q14 注入到会话里的上下文有多大 会不会超预算 | relevance | t18 | 2 | hybrid+semantic | 0 | 12.5ms |
| q15 timeline 的邻居是按什么顺序排的 | relevance | t20 | 1 | hybrid+semantic+fts | 0 | 10.1ms |
| q16 本地 embedding 模型是什么精度 多少维 | relevance | t23 | 1 | hybrid+fts | 0 | 9.2ms |
| q17 跨项目会不会看到别的仓库的记忆 | relevance | t22 | 1 | hybrid | 0 | 10.9ms |
| q18 质量基准集还剩什么没做 | relevance | t26 | 1 | hybrid+semantic | 0 | 9.9ms |
| q19 虚拟滚动 react-window 帧率优化 | empty | — | 返回 0 条 | — | 0 | 10.4ms |
| q20 401 token 刷新重放请求 | leakage | — | 返回 6 条 | hybrid+fts+semantic | 0 | 6.4ms |
| q21 Kubernetes Ingress 灰度发布配置 | empty | — | 返回 2 条 | semantic | 0 | 3.2ms |
| q22 支付回调 验签 对账差异 | empty | — | 返回 2 条 | semantic | 0 | 2.8ms |
| q23 iOS APNs 推送证书 续期 | empty | — | 返回 0 条 | — | 0 | 3.6ms |
| q24 Excel 导出 中文乱码 编码 | empty | — | 返回 1 条 | semantic | 0 | 2.0ms |
...
| q28 有没有真的跑通一整轮记忆生成 | relevance | t17 | 2 | semantic | 0 | 5.8ms |
| q29 两路检索结果是用什么算法合并的 | relevance | t21 | 1 | semantic | 0 | 5.3ms |
| q30 中文界面和中文提示是怎么支持的 | relevance | t24 | 1 | semantic | 0 | 5.2ms |
| q31 还有哪些类型错误没有修完 | relevance | t25 | 1 | hybrid+semantic | 0 | 9.7ms |
| q32 搜索词里带引号或括号会不会崩 | relevance | t01 | 1 | semantic | 0 | 5.3ms |
| q33 向量算不出来的时候搜索还能不能用 | relevance | t02 | 未命中 | semantic | 0 | 5.7ms |
| q34 报错日志会不会把模型原始输出写进去 | relevance | t04 | 1 | hybrid+semantic | 0 | 7.5ms |
| q35 改一次版本号要动几个文件 | relevance | t05 | 1 | hybrid+semantic | 0 | 5.8ms |
| q36 连接池里有个子进程死了会怎么处理 | relevance | t07 | 1 | semantic | 0 | 6.0ms |
| q37 模型输出解析不了会不会把这一轮丢掉 | relevance | t08 | 未命中 | semantic | 0 | 7.9ms |
| q38 单元测试用的数据库文件放在哪里 | relevance | t12 | 4 | hybrid+semantic+fts | 0 | 12.3ms |
| q39 安装时少拷了一个文件会有什么现象 | relevance | t13 | 未命中 | hybrid+semantic | 0 | 10.8ms |
| q40 会话开头塞进去的索引占多少字节 | relevance | t18 | 1 | semantic | 0 | 5.6ms |
| q41 两个不同仓库的记忆是怎么分开的 | relevance | t22 | 1 | hybrid+semantic | 0 | 10.4ms |
| q42 向量是多少维 用了什么量化 | relevance | t23 | 1 | semantic | 0 | 6.4ms |
