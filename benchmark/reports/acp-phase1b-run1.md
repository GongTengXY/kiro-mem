...
| 项 | 值 |
| --- | --- |
| 生成时间 | 2026-07-30T16:09:20.902Z |
| commit | `07f8c50` **（工作区有未提交改动）** |
| 命令行 | `bun run benchmark/run.ts --compressor=acp --protocol=semantic-en --report=benchmark/reports/acp-phase1b-run1.md --json=/tmp/acp1.json` |
...
- 合成结果：normal 30 / fallback 0，summarize 总耗时 139.3s
...
| bootstrap 构建 < 100ms 且在预算内 | 20.5ms / 3704B | ✅ |
...
| t19 | normal | next_steps 过度生成（参考）；事实缺失: 734ms / 2 秒 / REDACTED |
...
| x04 | normal | memory_type 不符（参考）；事实缺失: 58fps |
...
| hit@10 | 86.1% |
| MRR@10 | 0.722 |
| ├ MRR@10（tuned；排序敏感，阶段 1 主读数） | 0.935 |
| └ MRR@10（heldout；排序敏感，泛化读数） | 0.509 |
| recall@10 | 86.1% |
| R-precision（下界） | 61.1% |
...
| 跨 workspace 泄漏条数 | 0 |
| FTS-only 降级次数 | 0 / 42 |
| search p50 / p95 | 10.5ms / 15.4ms |
| bootstrap 当前 scope | 3704B / 20.5ms（预算 8192B） |
| bootstrap 另一 scope | 1325B |
...
| --- | --- | --- | --- | --- | --- | --- |
| q01 FTS 特殊字符查询报错是怎么修的 | relevance | t01 | 1 | hybrid+fts+semantic | 0 | 27.9ms |
| q02 search 为什么会降级成只用 FTS | relevance | t02 | 3 | hybrid+fts+semantic | 0 | 13.6ms |
| q03 hook 超时会不会拖慢 Kiro 启动 | relevance | t03,t19 | 1,3 | hybrid+fts+semantic | 0 | 12.5ms |
| q04 日志里会不会有用户 prompt 正文 | relevance | t04,t19 | 2,4 | hybrid+fts+semantic | 0 | 9.7ms |
| q05 版本号是在哪里统一管理的 | relevance | t05 | 1 | hybrid+semantic | 0 | 8.0ms |
| q06 MCP search 的 limit 最大能传多少 | relevance | t06 | 1 | hybrid+fts | 0 | 12.3ms |
| q07 ACP 连接池回收失败会不会导致任务永久挂住 | relevance | t07 | 1 | hybrid+fts+semantic | 0 | 13.6ms |
| q08 压缩一直失败最后会写成什么样的记录 | relevance | t08 | 1 | hybrid+semantic | 0 | 9.5ms |
| q09 本地 token 认证是怎么做到用户无感的 | relevance | t11 | 1 | hybrid+fts | 0 | 14.4ms |
| q10 跑测试会不会污染我真实的数据目录 | relevance | t12 | 1 | hybrid+semantic | 0 | 10.1ms |
| q11 安装完 Worker 起不来说模块找不到 | relevance | t13 | 1 | hybrid+fts+semantic | 0 | 15.4ms |
| q12 重新安装之后为什么还在跑旧代码 | relevance | t14 | 1 | hybrid+fts+semantic | 0 | 10.1ms |
| q13 怎么发现运行中的 Worker 不是当前安装的版本 | relevance | t15 | 1 | hybrid+semantic | 0 | 21.4ms |
| q14 注入到会话里的上下文有多大 会不会超预算 | relevance | t18 | 1 | hybrid+semantic | 0 | 14.5ms |
| q15 timeline 的邻居是按什么顺序排的 | relevance | t20 | 1 | hybrid+fts | 0 | 10.9ms |
| q16 本地 embedding 模型是什么精度 多少维 | relevance | t23 | 1 | hybrid+fts | 0 | 10.6ms |
| q17 跨项目会不会看到别的仓库的记忆 | relevance | t22 | 1 | hybrid+fts | 0 | 11.6ms |
| q18 质量基准集还剩什么没做 | relevance | t26 | 1 | hybrid+semantic | 0 | 10.5ms |
| q19 虚拟滚动 react-window 帧率优化 | empty | — | 返回 0 条 | — | 0 | 10.7ms |
...
| q25 purge 之后重装会不会残留旧表 | relevance | t09 | 2 | hybrid+fts+semantic | 0 | 10.0ms |
| q26 job 的类型名后来有没有改过 | relevance | t10 | 1 | hybrid+fts | 0 | 9.0ms |
| q27 干净机器上装完做过验收吗 | relevance | t16 | 未命中 | — | 0 | 5.2ms |
| q28 有没有真的跑通一整轮记忆生成 | relevance | t17 | 3 | fts+semantic | 0 | 11.2ms |
| q29 两路检索结果是用什么算法合并的 | relevance | t21 | 2 | fts+semantic | 0 | 11.7ms |
| q30 中文界面和中文提示是怎么支持的 | relevance | t24 | 1 | semantic+fts | 0 | 10.8ms |
| q31 还有哪些类型错误没有修完 | relevance | t25 | 1 | hybrid+semantic | 0 | 8.9ms |
| q32 搜索词里带引号或括号会不会崩 | relevance | t01 | 2 | fts+semantic | 0 | 9.1ms |
| q33 向量算不出来的时候搜索还能不能用 | relevance | t02 | 未命中 | — | 0 | 4.9ms |
| q34 报错日志会不会把模型原始输出写进去 | relevance | t04 | 1 | hybrid+semantic+fts | 0 | 11.5ms |
| q35 改一次版本号要动几个文件 | relevance | t05 | 1 | hybrid+fts+semantic | 0 | 8.4ms |
| q36 连接池里有个子进程死了会怎么处理 | relevance | t07 | 2 | hybrid+semantic | 0 | 8.4ms |
| q37 模型输出解析不了会不会把这一轮丢掉 | relevance | t08 | 未命中 | hybrid+fts | 0 | 12.0ms |
| q38 单元测试用的数据库文件放在哪里 | relevance | t12 | 3 | hybrid+semantic+fts | 0 | 13.0ms |
| q39 安装时少拷了一个文件会有什么现象 | relevance | t13 | 2 | fts+semantic | 0 | 11.7ms |
| q40 会话开头塞进去的索引占多少字节 | relevance | t18 | 未命中 | — | 0 | 4.6ms |
| q41 两个不同仓库的记忆是怎么分开的 | relevance | t22 | 1 | hybrid+fts+semantic | 0 | 10.3ms |
...
| q02 | relevance/tuned | en | 7 | 6 | 9 | 2 | 2 | 2 | 3 | 否 |
...
| q07 | relevance/tuned | en | 9 | 7 | 10 | 3 | 1 | 1 | 1 | 否 |
| q08 | relevance/tuned | en | 2 | 7 | 7 | 5 | 1 | 2 | 1 | 否 |
| q09 | relevance/tuned | en | 5 | 2 | 5 | 0 | 1 | 1 | 1 | 否 |
| q10 | relevance/tuned | en | 3 | 9 | 9 | 6 | 1 | 2 | 1 | 否 |
| q11 | relevance/tuned | en | 11 | 5 | 10 | 1 | 1 | 1 | 1 | 否 |
| q12 | relevance/tuned | en | 2 | 6 | 7 | 5 | 1 | 1 | 1 | 否 |
| q13 | relevance/tuned | en | 11 | 12 | 10 | 1 | 1 | 1 | 1 | 否 |
| q14 | relevance/tuned | en | 2 | 5 | 5 | 3 | 1 | 1 | 1 | 否 |
...
| q25 | relevance/heldout | en | 4 | 3 | 5 | 1 | 1 | 2 | 2 | 否 |
| q26 | relevance/heldout | en | 4 | 2 | 4 | 0 | 1 | 1 | 1 | 否 |
| q27 | relevance/heldout | en | 0 | 0 | 0 | 0 | — | — | — | 否 |
| q28 | relevance/heldout | en | 1 | 3 | 4 | 3 | — | 2 | 3 | 否 |
| q29 | relevance/heldout | en | 1 | 1 | 2 | 1 | — | 1 | 2 | 否 |
| q30 | relevance/heldout | en | 1 | 1 | 2 | 1 | — | 1 | 1 | 否 |
| q31 | relevance/heldout | en | 1 | 12 | 10 | 9 | 1 | 1 | 1 | 否 |
| q32 | relevance/heldout | en | 1 | 3 | 4 | 3 | — | 1 | 2 | 否 |
| q33 | relevance/heldout | en | 0 | 0 | 0 | 0 | — | — | — | 否 |
| q34 | relevance/heldout | en | 2 | 4 | 5 | 3 | 1 | 1 | 1 | 否 |
| q35 | relevance/heldout | en | 2 | 2 | 3 | 1 | 1 | 1 | 1 | 否 |
| q36 | relevance/heldout | en | 1 | 9 | 9 | 8 | — | 1 | 2 | 否 |
| q37 | relevance/heldout | en | 2 | 1 | 2 | 0 | — | — | — | 否 |
| q38 | relevance/heldout | en | 5 | 7 | 10 | 5 | — | 2 | 3 | 否 |
| q39 | relevance/heldout | en | 1 | 3 | 4 | 3 | — | 1 | 2 | 否 |
| q40 | relevance/heldout | en | 0 | 0 | 0 | 0 | — | — | — | 否 |
| q41 | relevance/heldout | en | 2 | 4 | 5 | 3 | 2 | 1 | 1 | 否 |
