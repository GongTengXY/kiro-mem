...
| 项 | 值 |
| --- | --- |
| 生成时间 | 2026-07-30T12:56:08.680Z |
| commit | `07f8c50` **（工作区有未提交改动）** |
| 命令行 | `bun run benchmark/run.ts --compressor=gold --protocol=raw --report=benchmark/reports/gold-phase1b-raw.md --json=/tmp/armA.json` |
...
| bootstrap 构建 < 100ms 且在预算内 | 15.6ms / 3096B | ✅ |
...
| 跨 workspace 泄漏条数 | 0 |
| FTS-only 降级次数 | 0 / 42 |
| search p50 / p95 | 9.1ms / 13.6ms |
| bootstrap 当前 scope | 3096B / 15.6ms（预算 8192B） |
...
| q03 hook 超时会不会拖慢 Kiro 启动 | relevance | t03,t19 | 3,2 | hybrid+semantic | 0 | 9.2ms |
...
| q08 压缩一直失败最后会写成什么样的记录 | relevance | t08 | 未命中 | — | 0 | 4.9ms |
| q09 本地 token 认证是怎么做到用户无感的 | relevance | t11 | 1 | hybrid+semantic | 0 | 11.1ms |
| q10 跑测试会不会污染我真实的数据目录 | relevance | t12 | 2 | semantic+fts | 0 | 10.7ms |
| q11 安装完 Worker 起不来说模块找不到 | relevance | t13 | 1 | hybrid | 0 | 12.3ms |
| q12 重新安装之后为什么还在跑旧代码 | relevance | t14 | 1 | hybrid+semantic | 0 | 10.6ms |
...
| q15 timeline 的邻居是按什么顺序排的 | relevance | t20 | 1 | hybrid+semantic | 0 | 12.2ms |
| q16 本地 embedding 模型是什么精度 多少维 | relevance | t23 | 1 | hybrid+semantic | 0 | 9.8ms |
...
| q20 401 token 刷新重放请求 | leakage | — | 返回 10 条 | hybrid+semantic | 0 | 7.6ms |
| q21 Kubernetes Ingress 灰度发布配置 | empty | — | 返回 0 条 | — | 0 | 2.8ms |
| q22 支付回调 验签 对账差异 | empty | — | 返回 0 条 | — | 0 | 2.2ms |
| q23 iOS APNs 推送证书 续期 | empty | — | 返回 0 条 | — | 0 | 3.3ms |
| q24 Excel 导出 中文乱码 编码 | empty | — | 返回 0 条 | — | 0 | 1.4ms |
| q25 purge 之后重装会不会残留旧表 | relevance | t09 | 1 | hybrid+semantic | 0 | 8.8ms |
| q26 job 的类型名后来有没有改过 | relevance | t10 | 1 | hybrid+semantic | 0 | 6.9ms |
| q27 干净机器上装完做过验收吗 | relevance | t16 | 1 | hybrid+semantic | 0 | 7.0ms |
| q28 有没有真的跑通一整轮记忆生成 | relevance | t17 | 未命中 | — | 0 | 4.3ms |
| q29 两路检索结果是用什么算法合并的 | relevance | t21 | 未命中 | — | 0 | 4.5ms |
| q30 中文界面和中文提示是怎么支持的 | relevance | t24 | 未命中 | — | 0 | 4.4ms |
| q31 还有哪些类型错误没有修完 | relevance | t25 | 1 | fts+semantic | 0 | 9.1ms |
| q32 搜索词里带引号或括号会不会崩 | relevance | t01 | 未命中 | — | 0 | 5.1ms |
...
| q38 单元测试用的数据库文件放在哪里 | relevance | t12 | 未命中 | hybrid+semantic | 0 | 13.1ms |
| q39 安装时少拷了一个文件会有什么现象 | relevance | t13 | 4 | hybrid+semantic | 0 | 11.1ms |
| q40 会话开头塞进去的索引占多少字节 | relevance | t18 | 未命中 | — | 0 | 5.5ms |
| q41 两个不同仓库的记忆是怎么分开的 | relevance | t22 | 1 | fts+semantic | 0 | 13.6ms |
| q42 向量是多少维 用了什么量化 | relevance | t23 | 未命中 | — | 0 | 5.5ms |
