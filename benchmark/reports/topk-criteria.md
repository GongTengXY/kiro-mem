...
#### 修订 B1（2026-08-05，**在看到 50,000 档读数之后**，由裁定作出）

原 §6.3 只写了「p95 < 300ms」「p99 < 500ms」和「≥200 样本」，**没写多次复现如何聚合**。
实测暴露了这个缺口：50,000 档的尾部延迟被机器负载污染（测量时 load average 5.0–5.8，
竞争进程含 kiro-cli 自身 66.6% CPU、Chrome 53.9% + 43.6%、WindowServer 42.4%），
`p95` 因此**跨 K 不单调**——K=200 出现过 372ms，而 K=1000 五次最差 286ms，
可是 K=200 做的工作严格更少。不同聚合口径给出不同的产品结论，所以口径必须由裁定定，
不能由我看着数字挑。

**裁定的口径（本节此后按此执行）**：

> S2 / S3 采用**「每次复现都需达标」**。不接受中位数，不接受「最小值 + 报告分布」。

**修订时点与强度**：晚于全部 50,000 档读数，因此预登记强度低于 A0，与候选池轮次的 B1、
FTS 修复轮的 B1 同级。抵消它的是三件事：修订由裁定作出而非我自行决定；口径是**收紧**
（严格口径比中位数与最小值都更难通过）；以及它同时否掉了我当时倾向的方案（∞ 不截断）。

**裁定同批给出的第二条，属于结构性理由而不是读数**：`semanticTopK = Infinity` 并**不是**
严格的有界内存实现。`src/server/observation-search.ts:596` 那两行在 `topK = Infinity` 时
让堆无上界增长，于是保留集 = 全部过 floor 项——本轮实测 50,000 档平均 **4,618**、
最坏 **14,830** 条。它解决的是"全部 blob 同时驻留"，没有解决"保留集随语料规模增长"。
只有有限 K 才真正满足 §1 承诺的 `O(块大小 + K)`。因此即使 ∞ 的 RSS 读数落在预算内，
它也不是本轮要交付的形态。

**当前落点（裁定）**：`semanticCandidatePool = Infinity` + `semanticTopK = 1000`；
`Infinity` + 不截断**不得发布**。
...
| B1 修订 | §6.3 聚合口径 =「每次复现都需达标」+ ∞ 不截断不得发布。时点 = **看到 50,000 档读数之后**，由裁定作出 |
| B1 checksum | `2285d0fa8cf5e1ee` |
| B1 修订时间 | `2026-08-05T12:27:47Z` |
